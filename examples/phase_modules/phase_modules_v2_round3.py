...
def m_kk_embedding():
    from systrophe.geometry.kk_embedding import kk_tower_masses
...
def m_anec_bound():
    from systrophe.qftcs.anec_bound import (
...
def m_cosmological_embedding():
    from systrophe.geometry.cosmological_embedding import (
...
def m_penrose_extraction():
    from systrophe.qftcs.penrose_extraction import penrose_efficiency_at
    vs = VanStockumInterior(omega=1.0, R=1.0)
    if not vs.is_supercritical():
        return None
    r_grid = np.linspace(1.05, 6.0, 25)
...
def m_page_curve_ctc():
    from systrophe.ctc.page_curve_ctc import (
