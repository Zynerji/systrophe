...
def m_aharonov_bohm():
    from systrophe.foundations.aharonov_bohm_ctc import aharonov_bohm_phase, enclosed_flux
...
def m_bh_pair_production():
    from systrophe.qftcs.bh_pair_production import schwinger_analog_rate
    from systrophe.geometry.vanstockum import VanStockumInterior
    vs = VanStockumInterior(omega=1.0, R=1.0)
    if not vs.is_supercritical():
        return None
...
def m_er_epr_pair():
    from systrophe.quantum_info.er_epr_pair import (
...
def m_holographic_complexity():
    from systrophe.quantum_info.holographic_complexity import volume_proxy_C, action_proxy_C
    from systrophe.geometry.vanstockum import VanStockumInterior
    vs = VanStockumInterior(omega=1.0, R=1.0)
    if not vs.is_supercritical():
        return None
...
def m_lp_dualities():
    from systrophe.lp.lp_dualities import s_dual_a
...
def m_monopole_on_cylinder():
    from systrophe.geometry.monopole_on_cylinder import dirac_quantization_condition
    from systrophe.geometry.vanstockum import VanStockumInterior
...
def m_twistor_lp():
    from systrophe.lp.twistor_lp import alpha_plane_at_r, twistor_inner_product
    from systrophe.geometry.vanstockum import VanStockumInterior
    vs = VanStockumInterior(omega=1.0, R=1.0)
    if not vs.is_supercritical():
        return None
...
def m_unruh_effect():
    from systrophe.qftcs.unruh_effect import combined_unruh_hawking_T
    from systrophe.geometry.vanstockum import VanStockumInterior
    vs = VanStockumInterior(omega=1.0, R=1.0)
    if not vs.is_supercritical():
        return None
