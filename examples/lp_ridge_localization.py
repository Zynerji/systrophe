...
from systrophe.catcher_v2 import scan_novelty_coherence_anomaly
...
def f_frame_dragging(r):
    from systrophe.frame_dragging import (
...
def f_cauchy_stability(r):
    try:
        from systrophe.cauchy_stability import cauchy_perturbation_growth
    except Exception:
        return np.zeros(3)
    try:
...
def f_unruh_effect(r):
    try:
        from systrophe.unruh_effect import unruh_temperature_at_radius
...
def f_acoustic_metric(r):
    from systrophe.acoustic_metric import acoustic_line_element_at_radius
    try:
...
def f_berry_phase_lp(r):
    try:
        from systrophe.berry_phase_lp import berry_phase_around_loop_at_radius
