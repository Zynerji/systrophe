"""Verification battery for the Grok 'updates.txt' continuation (2026-05-10).

The follow-up Grok chat (saved as ~/Desktop/updates.txt) advances from
...
# =====================================================================
# Grok: zeta_H(-3, a) = -a^4 / 4 + a^3 / 2 - a^2 / 4 + 1 / 120

def hurwitz_zeta_neg3_grok(a):
...
# Matches Grok's formula exactly.
test_values = [0.0, 0.5, 1.0, 1/3, 2/3, 1/6]
errors = []
for a in test_values:
    B4 = a**4 - 2 * a**3 + a**2 - 1/30
    ref = -B4 / 4
    grok = hurwitz_zeta_neg3_grok(a)
    errors.append(abs(ref - grok))
verified = max(errors) < 1e-15
record(
    "hurwitz_zeta_neg3",
    "Hurwitz zeta at s=-3: zeta_H(-3, a) = -a^4/4 + a^3/2 - a^2/4 + 1/120",
    verified,
    f"max |Grok form - Bernoulli form| over a in {test_values}: {max(errors):.2e}",
)
...
# =====================================================================
# Grok cites: <rho>_Casimir = -pi^2 h_bar c / (720 d^4)
...
# =====================================================================
# Grok claims: with L_eff = r_eff^2 (1 - (omega r_eff / R)^2 / 4) and
...
def horned_torus_L_eff(R=5.0, r_base=2.0, hf=1.0, omega=1.5, n=200):
    ...
    Reproduce Grok's model: r_eff(theta) = r_base * (1 + (hf - 1) * cos(theta)^2)
    creates a horn pinch on the inner equator. Compute L_eff per Grok's
...
# Grok claimed: 0 at omega <= 1.4; 0.20 at 1.5; 0.32 at 1.6; 0.41 at 1.8
...
record(
    ...
    "Horned torus L_eff = r^2 (1 - (omega r / R)^2 / 4) with hf=1 gives Grok-reported CTC fractions",
    matches,
    f"Computed: {fractions}. Grok: {expected}. Matches within 5% threshold: {matches}.",
)
...
# Grok declared "Converged to self-consistent solution!"
...
# fixed-point would have delta_step decreasing. Grok's iteration is in
...
record(
    ...
    "Grok claims the self-consistent iterator converges to a fixed point in <10 iterations",
    False,  # FALSIFIED
    ...
    "Grok's 'converged' verdict is incorrect; the iteration is simply walking past zero.",
)
...
# =====================================================================
# Grok: C(delta) = (1/720) sum_{b=0,1,2} zeta_H(-3, b/3 + gamma_eff(delta) / (2 pi))
# Verify the formula evaluates and check its delta dependence.

def topological_casimir_C(gamma_eff):
    return (1 / 720) * sum(
        hurwitz_zeta_neg3_grok(b / 3 + gamma_eff / (2 * np.pi))
        for b in (0, 1, 2)
    )
...
record(
    ...
    "Grok's topological Casimir coefficient C(delta) is well-defined and evaluates without divergence",
...
# =====================================================================
# If gamma_eff depends on delta linearly (Grok's chat is unclear about
...
record(
    ...
    "If ~1, C is essentially constant in delta (function of b/3 only) and Grok's "
...
# =====================================================================
# Grok: rho_throat = -h_bar c / d_eff^4 * C(delta)
...
record(
    ...
    "Grok cites is not derived in updates.txt; just asserted.",
)
...
# =====================================================================
# Grok: "When delta(t) is held near pi (the off state), the DCE flux
...
record(
    ...
    "DCE flux drops by >2 orders of magnitude near delta = pi (Grok's off-switch claim)",
    ratio > 100.0,
    f"|C(0)|/|C(pi)| = {ratio:.4f}. Grok claims >100. "
    ...
    "Casimir coefficient. Grok asserts this without derivation.",
)
...
# =====================================================================
# Grok's effective-separation formula. Dimensionally OK, but the physical
...
# it's Grok's own ansatz. Not testable as a pure math statement; flag.
record(
    "casimir_cavity_at_throat_ansatz",
    "Grok's identification of the ringhole throat as a Casimir cavity with effective separation 2 b (1 - lambda)",
    None,
    ...
    "energies with appropriate UV regularization. Grok skips (a), (b), (c) and "
...
record(
    ...
    "Grok's gravito-dynamical Casimir power formula derived as the time-derivative of the static throat energy",
    None,
...
def grok_dce_power(delta_arr, b_arr, lambda_fold=0.7):
    ...
    for i, d in enumerate(delta_arr):
        ...
        for bb in (0, 1, 2):
            ...
            s += hurwitz_zeta_neg3_grok(arg)
            sp += (hurwitz_zeta_neg3_grok(arg + eps) - hurwitz_zeta_neg3_grok(arg - eps)) / (2 * eps)
        C[i] = s / 720
        Cp[i] = sp / 720
...
P_arr = grok_dce_power(delta_t, b_t, lambda_fold=0.7)
peak_power = float(np.max(np.abs(P_arr)))
record(
    "grok_dce_toy_peak_power",
    "Grok's toy run: delta(t) = 0.5 + 0.3 sin t, b(t) = 1 + 0.1 sin 2t, lambda = 0.7 -> peak DCE power ~ 0.042",
    abs(peak_power - 0.042) < 0.03,
    f"Reproduced peak |P_DCE| = {peak_power:.4f} (Grok claimed ~ 0.042). "
    "Tolerance: 3 sigma of Grok's reporting precision.",
)
...
# =====================================================================
# Grok asserts this is "the first exact, controllable example where
...
# =====================================================================
# Grok provides c_s^2 ~ 1 - (alpha^2 / 4) sin^2(alpha u + gamma_eff) and a
...
record(
    ...
    None,
    "Grok's mapping is dimensionally consistent but skips the matter sector (no fluid "
...
def main() -> None:
    ...
    for c in CLAIMS:
        ...
        print(f"     {c['evidence']}")
        print()
    out_path = Path("examples") / "grok_updates_verification_results.json"
