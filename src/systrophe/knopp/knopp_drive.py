...
from systrophe.geometry.horn_toroidal_warp import (
...
from systrophe.geometry.tipler_krasnikov_hybrid import (
...
class KnoppDrive:
    ...
    def journey(self, distance: float, n_steps: int = 80):
        ...
        KnoppTraversalReport
            See `systrophe.knopp.knopp_traversal.KnoppTraversalReport`.
        """
    ...
    def bias(
        ...
        Runs a bias controller (see `systrophe.knopp.knopp_ratchet`) that swings the
        ...
        from systrophe.knopp.knopp_ratchet import WarpRatchetConfig, ratchet_traversal
    ...
    def asymmetry(
        ...
        """
        from systrophe.knopp.knopp_ratchet import WarpRatchetConfig, reverse_asymmetry
        cfg = WarpRatchetConfig(
    ...
    def pump_accounting(
        ...
        """
        from systrophe.knopp.knopp_ratchet import WarpRatchetConfig, casimir_pump_accounting
        cfg = WarpRatchetConfig(
    ...
    def energy_ledger(
        ...
        Returns a `BiasEnergyLedger` (see `systrophe.knopp.knopp_ratchet`) with the
        pump, borrow, and quantum-interest-repay accounts, the cost per unit
        directed displacement, and the irreducible Q -> inf floor.
        """
        from systrophe.knopp.knopp_ratchet import WarpRatchetConfig, bias_energy_ledger
        cfg = WarpRatchetConfig(
            drive=self._cfg, heading=float(heading),
            floor_pct=float(floor_pct), n_cycles=int(n_cycles),
        )
    ...
    def capacitor_accounting(self, floor_pct: float = 0.85):
        ...
        """
        from systrophe.knopp.knopp_ratchet import WarpRatchetConfig, capacitor_accounting
