...
\begin{abstract}
We report a single-round logical-versus-physical break-even crossing for the distance-5 rotated surface code's Z-memory on IBM Quantum's \texttt{ibm\_kingston} 156-qubit Heron-r2 processor. The experiment encodes $|0_L\rangle$ on a $5 \times 5$ data-qubit grid (25 data + 12 Z-stabilizer ancillas = 37 qubits), performs $n_{\mathrm{rounds}} \in \{1, 2, 4\}$ rounds of Z-syndrome extraction, and decodes the syndrome difference history via minimum-weight perfect matching (MWPM) on a graph built with \texttt{networkx.min\_weight\_matching}. The MWPM-decoded logical recovery rate at $n_{\mathrm{rounds}} = 1$ is $P(\hat L_{\mathrm{MWPM}} = 0) = 0.8816$, compared with a wall-clock-matched bare-qubit baseline of $P_{\mathrm{bare}}(Z = 0) = 0.8528$. The difference $+0.0288$ is $4.0\sigma$ above zero at $4096$ shots. This is the first logical-error-rate below physical-error-rate demonstration in our \Systrophe{} Heron-r2 QEC experimental program, and supersedes the $d=3$ Steane sub-threshold baseline reported in \texttt{paper/steane\_logical\_qubit.pdf}. At higher $n_{\mathrm{rounds}}$ the MWPM advantage shrinks (n=2: $-0.033$ pp; n=4: $-0.020$ pp) because our shortest-path decoder is approximate. We do \emph{not} claim to beat the SOTA QEC threshold demonstrations of Google Quantum AI Willow (distance-7, sustained below-threshold) \cite{willow} or IBM bivariate-bicycle codes \cite{bb_paper}; we report a clean single-round break-even crossing on Heron-r2 with a documented, reproducible MWPM pipeline.
\end{abstract}
...
\toprule
$n_{\mathrm{rounds}}$ & Greedy & MWPM & Bare baseline & MWPM $-$ bare & Significance \\
\midrule
1 & $0.6990$ & $\bm{0.8816}$ & $0.8528$ & $\bm{+0.0288}$ & $\bm{4.0\sigma}$ \\
2 & $0.6013$ & $0.7429$ & $0.7754$ & $-0.0325$ & $4.0\sigma$ \\
4 & $0.5337$ & $0.6821$ & $0.7024$ & $-0.0203$ & $2.4\sigma$ \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{itemize}[leftmargin=*,itemsep=0pt]
\item \textbf{At $n_{\mathrm{rounds}} = 1$, the MWPM-decoded $d = 5$ surface code Z-memory rate is $0.8816$, exceeding the wall-clock-matched bare-qubit baseline of $0.8528$ by $+2.88$ percentage points ($4.0\sigma$ at 4096 shots).}
\item The greedy decoder, by comparison, gives only $0.699$ at $n_{\mathrm{rounds}} = 1$ --- $15.4$ pp below bare. MWPM is critical for recovering the $d = 5$ advantage.
\item At $n_{\mathrm{rounds}} \ge 2$, MWPM still loses to bare, with the gap shrinking as $n$ grows (from $-3.25$ pp at $n = 2$ to $-2.03$ pp at $n = 4$). The MWPM advantage at single-round measurement does not yet sustain across multiple rounds.
\end{itemize}

\subsection{Why the advantage does not extend past $n = 1$}

Two reasons:
\begin{enumerate}[leftmargin=*,itemsep=0pt]
\item The shortest-path X-flip correction we apply between matched stab pairs is a one-qubit flip, not the full multi-qubit chain along the lattice-Manhattan shortest path. For weight-2 distances this is correct; for larger distances it under-corrects, growing in error rate as the round count and per-round error rate grows.
\item Z-syndrome-only memory does not detect phase errors that flip the X-eigenstate of the measurement ancillas during their measure-reset cycle. These propagate forward and accumulate.
\end{enumerate}

A full fault-tolerant MWPM decoder with proper 3D syndrome graphs and boundary nodes would likely extend the advantage further. We defer this to a follow-up study.
