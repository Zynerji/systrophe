...
\section{Open speculative items}

Six ansatz-level claims from the original Grok-chat analysis remain
not directly testable from inside the codebase and are documented
in \texttt{docs/INTERPRETATIONS.md}:

\begin{enumerate}
\item $\mathbb{Z}_3$ cover physically realises a wormhole throat
(needs gluing map);
\item Casimir negative energy replaces exotic matter (needs cavity
geometry);
\item Self-consistent $\delta$-iteration selects the chronology-
protecting phase (NK solver ready; needs $F(\delta)$);
\item M\"obius monodromy equals a U(1) Wilson loop (needs gauge
field);
\item DCE off-resonance photon flux (needs cavity $Q$ + modulation);
\item Throat cavity is the natural QFT vacuum (needs vacuum-
selection criterion).
\end{enumerate}

Each item is an open research project given external input.

\section{Summary and outlook}

\textsc{Systroph\=e} v0.13 provides the smallest open-source toolkit
...
the author's. \texttt{ultrareview} cycles caught several
mathematical errors in the early Grok-chat verification battery.
