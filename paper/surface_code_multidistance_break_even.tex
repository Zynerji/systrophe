...
\title{\Large\textbf{Sustained multi-distance QEC break-even on Heron-r2: $d = 5$ and $d = 7$ rotated surface codes with Dijkstra-MWPM decoding, reproduced across two chips}\\
\large{Distance-scaling signature on \texttt{ibm\_kingston} ($d = 7$, $+43.2\sigma$ at $n = 1$) and cross-chip reproducibility on \texttt{ibm\_marrakesh} ($d = 5$, $+22.6\sigma$ at $n = 4$)}}
